 ...
 */
export function HeroBackdrop({ className, imageSrc }: HeroBackdropProps = {}) {
  if (imageSrc) {
    return (
      <div
        className={cn(
          "absolute inset-0 size-full overflow-hidden bg-black select-none pointer-events-none",
          className
        )}
      >
        <Image
          src={imageSrc}
          alt="Catenate backdrop"
          fill
          priority
          sizes="100vw"
  ...
  return (
    <div
      className={cn(
        "absolute inset-0 size-full overflow-hidden select-none pointer-events-none",
        className
      )}
    >
      <svg
